...
import { getArticle, IArticle } from "@/api/article"
...
const ArticleDetail = ({ router }: WithRouterProps) => {
    ...
    const fetchDetail = async () => {
        setLoading(true)
        const result = await getArticle(Number(router.query['id']))
        setDetail(result)
        setLoading(false)
    }
        ...
        <div className="max-w-4xl py-5">
            ...
            <div className="p-5 bg-white markdownBody dark:bg-dark-600">
                <Skeleton
                    ...
                    animation
                >
                    {detail?.content! && <Viewer value={detail.content} plugins={plugins}></Viewer>}
                </Skeleton>
            </div>
